"""
Script para entrenar un modelo LightGBM con selección de características (RFE)
con el objetivo de detectar puntos de soldadura defectuosos (pegados).
Este script NO utiliza SMOTE, en su lugar, el LightGBM se entrena con el
parámetro `is_unbalanced=True` para manejar el desbalanceo de clases.
...
3.  Definición de un pipeline que:
    ...
    c. Entrena un modelo LGBMClassifier con el parámetro `is_unbalanced=True`.
...
7.  Guardado del pipeline completo (Scaler + Selector + LightGBM) y el umbral.
"""
...
def paso_3_entrenar_modelo(X_train, y_train, n_splits, fbeta, random_state):
    """
    *** LÓGICA CENTRAL: RFE + LightGBM (Balanceado) ***
    Configura y ejecuta GridSearchCV en un pipeline con RFE y LightGBM.
    """
    print("Iniciando búsqueda de hiperparámetros para RFE + LightGBM...")
    ...
    pipeline_lgbm = Pipeline(steps=[
        ...
        ('model', LGBMClassifier(           # 3. Modelo LightGBM
           objective="binary",
            boosting_type="gbdt",
            random_state=RANDOM_STATE_SEED,
            n_estimators=300,       # valor razonable, afinado vía grid
            verbose=-1,
            # Parámetro clave para manejar el desbalanceo de clases.
            # Internamente, LightGBM ajusta los pesos de las clases de forma inversa
            # a su frecuencia, similar a `class_weight='balanced'`.
            is_unbalanced=True
    ...
    print(f"GridSearchCV (RFE+LightGBM) probará {total_combinaciones} combinaciones.")
    ...
    print("Entrenamiento (GridSearchCV) de RFE + LightGBM completado.")
...
def paso_4_evaluar_importancia_y_umbral_defecto(mejor_modelo, X_test, y_test, feature_names):
   ...
   # --- 1. Recuperar nombres correctos tras la selección ---
    ...
    print("\nImportancia de las características seleccionadas para el modelo LightGBM:")
    print(df_importancias.sort_values(by='importancia', ascending=False))
    ...
    ax.set_title('Importancia de Características (RFE + LightGBM)')
    plt.tight_layout()
    plt.show()
    ...
    # *** CORRECCIÓN ***: Título del gráfico
    titulo = "Matriz de Confusión - LightGBM (Balanceado) (Umbral = 0.5)"
...
def paso_6_evaluacion_final_y_guardado(mejor_modelo, X_test, y_test, scaler, optimal_threshold, feature_names):
    ...
    # *** CORRECCIÓN ***: Título del gráfico
    titulo = f"Matriz de Confusión - LightGBM (Balanceado) (Umbral Óptimo = {optimal_threshold:.4f})"
    ...
    plt.figure()
    plt.plot(fpr, tpr, label=f"LightGBM (Balanceado) (AUC = {auc_score:.4f})")
...
def main():
    ...
    # PASO 3: Entrenar el pipeline RFE + LightGBM
