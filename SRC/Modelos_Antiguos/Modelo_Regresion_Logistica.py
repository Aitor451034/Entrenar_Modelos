...
# ==============================================================================
    def leer_archivo():
        ...
        ruta_csv = r"C:\Users\U5014554\Desktop\EntrenarModelo\DATA\Pruebas.csv"
        try:
...
# ==============================================================================
# 3. ¡MODIFICADO! Búsqueda eficiente con LogisticRegressionCV
# ==============================================================================
print("Iniciando búsqueda eficiente de lambda (C)...")
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# Definir el scorer para F2-score (Tu código)
fbeta = 2
f2_scorer = make_scorer(fbeta_score, beta=fbeta)

# CAMBIO: Lista de lambdas logarítmica (más eficiente) y más corta.
# 100 valores entre 0.0001 (10^-4) y 10 (10^1)
lista_lambdas_log = np.logspace(-4, 1, 1000)

# CAMBIO: Usamos LogisticRegressionCV en lugar de GridSearchCV.
# Es la herramienta optimizada EXACTAMENTE para esta tarea.
# Es muchísimo más rápida.
#
# Añadimos 'class_weight="balanced"' para dar más importancia
# a la clase minoritaria (defectos), lo cual ayuda a maximizar F2.
mejor_modelo = LogisticRegressionCV(
    Cs=lista_lambdas_log,       # Lista de 'C' a probar (plural)
    cv=skf,                     # El K-Fold estratificado
    scoring=f2_scorer,          # ¡Tu scorer F2!
    penalty="l1",               # Regularización L1 (Lasso)
    solver="liblinear",         # Solver para L1
    tol=1e-5,
    max_iter=3000,              # Reducido de 10000 (gracias al escalado)
    random_state=42,
    class_weight="balanced",    # <-- CLAVE: Ayuda a priorizar F2
    n_jobs=-1                   # Usar todos los núcleos
)

# Entrenamos el modelo. Esto hace la búsqueda y el re-entrenamiento final.
# Tu variable 'mejor_modelo' ya estará lista y entrenada.
mejor_modelo.fit(X_train, y_train)
print("Entrenamiento y búsqueda de C completados.")

# (Hemos eliminado el 'joblib.dump' de aquí porque ahora guardaremos
#  todo junto al final: modelo + scaler + umbral)

# ==============================================================================
# 4. Resultados del modelo (Ajustado a LogisticRegressionCV)
# ==============================================================================
# LogisticRegressionCV guarda el mejor C (lambda) en el atributo .C_[0]
mejor_lambda = mejor_modelo.C_[0]
print(f"Mejor lambda (C): {mejor_lambda}")

# Los scores se guardan en el atributo .scores_
# (Tomamos el primer (y único) valor del dict, calculamos la media por 'C'
# y encontramos el score máximo)
mejor_score = np.max(np.mean(list(mejor_modelo.scores_.values())[0], axis=0))
...
# ==============================================================================
df_coeficientes1 = pd.DataFrame({'predictor': X_train.columns,'coef': mejor_modelo.coef_.flatten()})
...
#    Usamos el mismo 'skf' (cv=5) que el LogisticRegressionCV
...
# Aplicar el umbral óptimo (calculado arriba) al conjunto de prueba X_test
print("Aplicando umbral óptimo al conjunto de prueba...")
...
# ==============================================================================
# 9.Guardar modelo, umbral Y SCALER
# ==============================================================================
