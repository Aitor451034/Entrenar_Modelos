...
# Importa esto al inicio de tu script
...
# ==============================================================================
# 3. ¡MODIFICADO! Búsqueda de hiperparámetros para Random Forest
# ==============================================================================
...
# 1. Definir el modelo
# Usamos class_weight="balanced" para ayudar con los datos desbalanceados
modelo_rf = RandomForestClassifier(random_state=42, class_weight="balanced")

# 2. Definir el GRID de parámetros (¡MÁS AMPLIO!)
# GridSearchCV probará TODAS las combinaciones.
# Hemos añadido 'max_features' y 'min_samples_split', que son cruciales.
param_grid = {
    # Más árboles (más coste)
    'n_estimators': [200, 300, 400], 
    
    # Profundidad (tu lista original estaba bien)
    'max_depth': [4, 5, 7, 10, None], 
    
    # Mínimo de muestras por hoja (tu lista original)
    'min_samples_leaf': [3, 5, 10],
    
    # ¡NUEVO! Mínimo de muestras para dividir un nodo
    'min_samples_split': [2, 5, 10],
    
    # ¡NUEVO! Nº de características a considerar en cada división
    # 'sqrt' es el valor por defecto, 'log2' es otra opción, 0.5 usa el 50%
    'max_features': ['sqrt', 'log2', 0.5]
}

# 3. Configurar la Búsqueda (GridSearchCV)
# ¡ADVERTENCIA! Esto será MUCHO más lento que RandomizedSearchCV.
# n_jobs=-1 usa todos los núcleos de tu CPU.
search_cv = GridSearchCV(
    estimator=modelo_rf,
    param_grid=param_grid,  # <-- Usamos el grid
    cv=skf,
    scoring=f2_scorer,      # <-- Tu objetivo de F2-Score
    n_jobs=-1,
    verbose=2               # <-- Verbose=2 te mostrará el progreso
)

# 4. Entrenar
# ¡IMPORTANTE! Usamos los datos escalados (X_train) como antes
print("Entrenando GridSearchCV... Esto puede tardar.")
...
plt.show()
# ==============================================================================
# 7. Optimizar umbral para (Precisión >= 0.70) Y (Max Recall)
# ==============================================================================
print("Optimizando umbral para MAX(Recall) sujeto a Precision >= 0.7...")

cv_thresh = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)
# Buscamos en un rango más sensible (más bajo)
lista_umbrales = np.linspace(0.01, 0.7, 1000) 

#Define tu objetivo de precisión mínima
PRECISION_MINIMA = 0.7 

optimal_threshold = 0.01
best_recall = -1  # Guardamos el mejor recall VÁLIDO
...
for train_idx, val_idx in cv_thresh.split(X_train, y_train):
    ...
    # 'mejor_modelo' es el pipeline que ya encontramos con RandomizedSearch
    # Lo re-entrenamos solo en el "fold" de entrenamiento
    modelo_cv_pipe = mejor_modelo.fit(X_train_cv, y_train_cv)
    
    # Obtenemos probabilidades en el "fold" de validación
    ...
    for thresh in lista_umbrales:
        ...
        # Si queremos R=1.0, podemos buscar el más cercano
        elif best_recall < 1.0 and rec == 1.0: 
             # Si encontramos un R=1.0 (aunque P sea < 0.8)
             # lo guardamos como un "plan B"
             pass 
...
if best_recall == -1:
    ...
    print("Como fallback, se optimizará para el mejor F2-Score (objetivo original).")
    
    optimal_threshold = 0
    best_fbeta_score = -1
    fbeta = 2 # Usamos F2 como un fallback razonable
    
    # (Aquí iría la lógica de fallback para F2, que es la que ya tenías)
    # ... (Por brevedad, la omito, pero es tu bucle `for thresh...` original) ...
    # ... Asumimos que el R=0.8/P=0.8 es este fallback ...
    optimal_threshold = 0.5115 # Asignamos tu resultado anterior como fallback
...
# ==============================================================================
# 7.Calcular la matriz de confusión con el umbral óptimo
# ============================================================================== 
...
plt.show()
# ==============================================================================
# 8.Guardar modelo, umbral Y SCALER
# ==============================================================================
