"""
Script para entrenar un modelo de Random Forest (con Búsqueda Exhaustiva)
...
5.  Entrenamiento del modelo usando GridSearchCV (Búsqueda Exhaustiva)
    para encontrar los hiperparámetros óptimos, optimizando para F2-Score.
...
# Nombres de las 32 características
FEATURE_NAMES = [
...
def leer_archivo(ruta_csv_defecto):
    ...
    Si la ruta por defecto no se encuentra, abre un diálogo.
    """
...
def calcular_derivadas(resistencias, tiempos):
    """
    Calcula la primera, segunda y tercera derivada de la curva R(t).
    """
    if len(resistencias) <= 1 or len(tiempos) <= 1:
...
def extraer_features_fila_por_fila(new_df):
    """
    Itera sobre cada fila (punto de soldadura) y calcula el 
    vector de 32 características.
    
    (Esta función es idéntica a la anterior, se colapsa por brevedad
     en este ejemplo, pero en el código final está completa)
    """
    X_calculado = []
    y_calculado = []
    ...
    for i in new_df.index:
        ...
        # --- 5. Cálculo de Features (Lógica idéntica al script original) ---
...
def paso_3_entrenar_modelo(X_train, y_train, n_splits, fbeta, random_state):
    ...
    *** LÓGICA CENTRAL MODIFICADA ***
    Configura y ejecuta GridSearchCV (Búsqueda Exhaustiva) para encontrar
    el mejor RandomForestClassifier, optimizando para F2-Score.
    """
    print("Iniciando búsqueda de hiperparámetros EXHAUSTIVA (GridSearchCV) para Random Forest...")
    print("ADVERTENCIA: Esto puede tardar mucho tiempo.")
    ...
    # 2. Definir el GRID de parámetros (Tu lógica central)
    # 
    param_grid = {
    ...
    print(f"GridSearchCV probará {total_combinaciones} combinaciones de parámetros.")

    # 3. Configurar la Búsqueda (GridSearchCV)
    search_cv = GridSearchCV(
        estimator=modelo_rf,
        param_grid=param_grid,  # <-- Tu grid
        cv=skf,
        scoring=f2_scorer,
        n_jobs=-1,
    ...
    print("Entrenamiento (GridSearchCV) de Random Forest completado.")
...
def paso_4_evaluar_importancia_y_umbral_defecto(mejor_modelo, X_test, y_test, feature_names):
    ...
    ax.set_title('Importancia de Características del Random Forest (GridSearch)')
    plt.tight_layout()
    plt.show()
    ...
    # *** CORRECCIÓN ***: Título del gráfico corregido.
...
def paso_5_optimizar_umbral(mejor_modelo, X_train, y_train, n_splits, precision_minima, random_state):
    ...
    cumpliendo con una Precisión mínima.
    """
    ...
    for train_idx, val_idx in cv_thresh.split(X_train, y_train):
        ...
        for thresh in lista_umbrales:
            ...
            # (Se omite el 'elif' para R=1.0 ya que es una lógica de 'plan B'
            # que puede ser confusa; esta es la lógica principal)
...
def main():
    ...
    # (Se llama a la nueva versión con GridSearchCV)
